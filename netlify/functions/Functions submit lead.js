...
// Replaces contact.js as the contact form's submit endpoint.
// Saves the submission as a "lead" in Netlify Blobs (so it can show up
// in admin's New Leads tab) AND emails the owner, same as before.
...
exports.handler = async (event) => {
  ...
  try {
    const { name, phone, email, subject, message, photoCount } = JSON.parse(event.body);

    if (!name || !email || !message) {
      return { statusCode: 400, headers, body: JSON.stringify({ error: 'Missing required fields.' }) };
    }

    // ── Save as a lead in Blobs ──
    ...
    const leadId = 'ld_' + Math.random().toString(36).substring(2, 10) + Date.now().toString(36).substring(-4);
    const lead = {
      leadId,
      name,
      phone: phone || '',
    ...
    // ── Email the owner, same format as before plus a photo line ──
    // This whole block is wrapped so that ANY email problem — missing key,
    // Resend being down, etc. — never costs us the lead. The lead is
    // already safely saved above; email is a nice-to-have on top of that.
    ...
    const photoLine = lead.photoCount > 0
      ? `<tr><td style="padding:10px 0;border-bottom:1px solid #222"><span style="font-size:10px;color:#555;text-transform:uppercase;letter-spacing:1.5px;font-weight:700">Photos</span><br><span style="color:#f5f5f0;font-weight:600;font-size:15px">📷 ${lead.photoCount} photo${lead.photoCount===1?'':'s'} attached — view in admin</span></td></tr>`
      : '';
...
<!DOCTYPE html><html><body style="margin:0;padding:0;background:#0a0a0a;font-family:Arial,sans-serif">
  ...
  <table width="100%" cellpadding="0" cellspacing="0" style="max-width:600px;margin:0 auto">
    ...
    try {
      if (!RESEND_API_KEY) {
        console.error('Email service not configured (RESEND_API_KEY missing) — lead saved, email skipped.');
      } else {
        ...
        if (!res.ok) console.error('Resend error (non-fatal, lead still saved):', JSON.stringify(data));
      }
    } catch (emailErr) {
      // Lead is already saved — a failed email shouldn't fail the whole request,
      // since the lead will still show up in admin's New Leads tab either way.
    ...
    console.error('submit-lead.js error:', err.message);
